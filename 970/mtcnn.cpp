...
Rect mtcnn::findFace(Mat &image){
    ...
    Rect temp_3;
    if(count<1)return temp_3;
    refineAndSquareBbox(thirdBbox_, image.rows, image.cols);
    nms(thirdBbox_, thirdBboxScore_, nms_threshold[2], "Min");
    vector<struct Bbox>::iterator it = thirdBbox_.begin();
    Rect rect_temp;
	if ((*it).exist)
	{

		float rect_width = (*it).x2 - (*it).x1;
		float rect_height = (*it).y2 - (*it).y1;
	        rect_temp = Rect((*it).y1 - rect_height*1.3, (*it).x1 - rect_width*0.7, rect_height * 4, rect_width * 10);
	

  /*  for(; it!=thirdBbox_.end();it++){
        if((*it).exist){
	...
	if (rect_temp.y < 0)
		rect_temp.y = 0;
	if ((rect_temp.x + rect_temp.width)>1280)
		rect_temp.width = 1280 - rect_temp.x;
	if ((rect_temp.y + rect_temp.height)>720)
		rect_temp.height = 720 - rect_temp.y;
	}
    firstBbox_.clear();
